...
export function TicketDetailPage() {
  ...
  useEffect(() => {
    ...
    async function fetchTicket() {
      ...
      // Fetch profiles for submitter + assignee
    ...
    const unsubscribe = onSnapshot(commentsQuery, async (snap) => {
      const comments = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Comment));
      setTicketComments(comments);

      // Fetch any new commenter profiles
  ...
  const formatTs = (ts: Comment['createdAt']) => {
    if (!ts) return '';
              ...
              {ticketComments.map((comment) => {
                ...
                return (
                  <div key={comment.id} className="flex gap-4">
                    <img
                      src={commentUser?.photoURL || `https://ui-avatars.com/api/?name=User&background=1B4332&color=D4A843`}
                      alt=""
                    ...
                    <div className="flex-1">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xs text-gray-500">{formatTs(comment.createdAt)}</span>
                      </div>
                      <div className="mt-1 text-gray-700 bg-gray-50 p-3 rounded-lg rounded-tl-none border border-gray-100">
                        {comment.body}
                      </div>
                    </div>
                  </div>
              ...
              <form onSubmit={handleAddComment} className="flex items-end gap-3">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full border-gray-300 rounded-lg shadow-sm focus:ring-brand-dark focus:border-brand-dark sm:text-sm border p-3 pr-12 resize-none"
                    rows={2}
                  />
                ...
                <button
                  ...
                  className="p-3 bg-brand-dark text-white rounded-lg hover:bg-[#153427] disabled:opacity-50 transition-colors shadow-sm"
                >
        ...
        <div className="w-full lg:w-80 space-y-6">
          <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div>
                <span className="block text-xs font-medium text-gray-500 uppercase mb-1">Type</span>
                <span className="text-sm text-gray-900 font-medium">{ticket.type}</span>
              </div>
              <div>
              ...
              <div>
                <span className="block text-xs font-medium text-gray-500 uppercase mb-1">Created</span>
                <span className="text-sm text-gray-900">{ticket.createdAt?.toDate().toLocaleDateString()}</span>
              </div>
            </div>
          </div>
