...
export function AdminSettingsPage() {
  ...
  const [inviteForm, setInviteForm] = useState({ name: '', email: '', role: 'user' as 'superadmin' | 'admin' | 'user' });
  ...
  const handleBulkImport = async () => {
    ...
    setBulkResult(null);
    try {
      // Parse emails from textarea (newlines, commas, spaces all OK)
      const raw = bulkEmails
        .split(/[\s,;]+/)
              ...
              {profiles.map((profile) => (
                <tr key={profile.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      value={profile.role}
                      onChange={(e) => updateUserRole(profile, e.target.value as Profile['role'])}
                  ...
                  <tr key={type.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 min-w-[200px]">
                      <AssigneeSelector
                        ...
                        admins={profiles.filter((p) => p.role === 'admin' || p.role === 'superadmin')}
                      />
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-dark"
                />
              </div>
              <div>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-5 py-2 text-sm font-medium rounded-lg bg-brand-dark text-white hover:bg-[#153427] disabled:opacity-50 transition-colors"
              >
