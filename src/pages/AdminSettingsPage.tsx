...
export function AdminSettingsPage() {
  ...
  const addRequestType = async () => {
    ...
    if (!name?.trim()) return;
    const newDoc = await addDoc(collection(db, 'requestTypes'), {
      name: name.trim(),
      defaultAssigneeId: null,
      active: true,
      createdAt: serverTimestamp(),
    });
    setRequestTypes((prev) => [...prev, { id: newDoc.id, name: name.trim(), defaultAssigneeId: null, active: true }].sort((a, b) => a.name.localeCompare(b.name)));
  };
                ...
                <tr key={profile.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button
                      ...
                      className="text-brand-dark hover:text-brand-gold text-xs font-medium transition-colors"
                    >
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-white">
              <tr>
                {['Type Name', 'Default Assignee', 'Status', 'Actions'].map((h) => (
                  <th key={h} scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {h}
                  </th>
                ))}
                ...
                return (
                  <tr key={type.id}>
                      ...
                      {editingType === type.id ? (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            autoFocus
                          />
                          <button onClick={() => saveTypeName(type)} className="text-emerald-600 hover:text-emerald-800">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {defaultAssignee ? defaultAssignee.name : 'Unassigned'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() => toggleRequestTypeActive(type)}
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium cursor-pointer ${type.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                      >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                      <button
                        onClick={() => { setEditingType(type.id); setEditTypeName(type.name); }}
                        className="text-gray-400 hover:text-brand-dark transition-colors inline-block"
                      >
